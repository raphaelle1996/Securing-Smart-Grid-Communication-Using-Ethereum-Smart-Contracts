...
contract Simulation {
    ...
    function requestEntry(uint32 smId, string key) public {
        ...
        //we don't have to check if it isn't already be a Node becuase once accepted we do not sent hasRequested to false...
...
contract Communication {
    ...
    //events
    event DataSent(address _from, bytes32 indexed _timestamp, string _value);
    event LoadBalancingSent(address indexed _to, string _value);

    //send the Data
    //timestamp should follow certain convention yyyymdhm
    function sendData(bytes32 timestamp, string value) public {
    ...
    function fixData(address to, string value) public {
        require(msg.sender == S.utility());
        LoadBalancingSent(to, value);
    }

    //constructor 
    //takes the simulation contract to refer to it later
